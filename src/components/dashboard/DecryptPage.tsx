...
export function DecryptPage() {
  ...
  const handleDecrypt = async () => {
    ...
    try {
      ...
      let result = formData.encryptedMessage;
      
      // Reverse AES simulation
      result = atob(result);
      if (result.includes("_AES_" + formData.aesKey)) {
        result = result.replace("_AES_" + formData.aesKey, "");
      } else {
        throw new Error("Invalid AES key");
      }
      
      // Reverse Vigenère simulation
      result = atob(result);
      if (result.includes("_VIG_" + formData.vigenereKey)) {
        result = result.replace("_VIG_" + formData.vigenereKey, "");
      } else {
        throw new Error("Invalid Vigenère key");
      }
      ...
      result = result.split('').map(char => {
        if (char.match(/[a-z]/i)) {
          ...
          return String.fromCharCode(((code - base - formData.caesarShift + 26) % 26) + base);
        }
        return char;
      }).join('');
